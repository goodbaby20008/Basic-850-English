...
便携版最低环境为：Intel/AMD 设备使用 Windows 10 或更新版本，ARM 设备使用 Windows 11 或更新版本，且使用 Chrome 或 Edge 的当前稳定版；启动器仅依赖系统自带的 Windows PowerShell 5.1。建议使用 Windows 11、最新版 Chrome，并保持 Windows Update 可用。若音标“单音”能播放，而字母、单词或例句没有声音，请在网站中点“声音检测”，再运行便携版根目录的 `修复英语发音.cmd`；它会补充英语（英国、美国）的基础和文本转语音组件，不会改变 Windows 显示语言。完成后彻底重开 Chrome。
...
- 字母名、单词和例句采用浏览器/操作系统的合成语音。UK/US 会优先选择对应地区音色；设备没有可用英语音色时，网站会明确提示并提供“声音检测”和 Windows 自助修复方法。
