...
test("the shared shell carries Ideal City branding and keeps typing translations readable", async () => {
  ...
  assert.match(source, /MOBILE_NAV_IDS:[^=]*=\s*\[[^\]]*"about"/s);
  assert.match(source, /window\.scrollTo\(\{\s*top:\s*0,\s*left:\s*0,\s*behavior:\s*"auto"\s*\}\)/s);
  assert.match(source, /focus\(\{\s*preventScroll:\s*true\s*\}\)/s);
