...
test("build emits one-click Windows launcher with a complete static site", async () => {
  ...
  await Promise.all([launcherUrl, serverUrl, guideUrl, siteIndexUrl, wordsUrl].map((url) => access(url)));
...
test("Windows-facing UTF-8 files carry a BOM for Windows PowerShell 5.1", async () => {
  const files = [
    new URL("./%E5%90%AF%E5%8A%A8%E8%8B%B1%E8%AF%AD%E5%AD%A6%E4%B9%A0%E6%95%99%E6%9D%90.cmd", packageRoot),
